...
public class SegTree1 {
	...
	static class SegTree{
		...
		public void push(int node, int low, int high) {
			if(lazy[node]==0||(node<2*size&&node>=size)) {
				return;
			}
			if(low<high) {
		...
		public void increment(int node, int node_low, int node_high, int query_low, int query_high, int amount) {
			if(query_low<=node_low&&node_high<=query_high) {
				tree[node]+=(node_high-node_low+1)*amount;
				return;
			}
